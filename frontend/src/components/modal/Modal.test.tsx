...
const mockedActionCacheService = ActionCacheService as jest.Mocked<
  typeof ActionCacheService
>;
...
describe("Modal", () => {
  ...
  beforeEach(() => {
    ...
    if (mockedActionCacheService.executeAction) {
      mockedActionCacheService.executeAction.mockResolvedValue({
        success: true,
      });
    }
  });
  ...
  describe("P0 - Tests critiques", () => {
    test("should render modal in document body when isOpen=true", () => {
      render(<Modal isOpen={true} onClose={jest.fn()} type="student" />);
    ...
    test("should not render modal when isOpen=false", () => {
      render(<Modal isOpen={false} onClose={jest.fn()} type="student" />);
  ...
  describe("P1 - Tests critiques", () => {
    ...
    test("should render modal with rdv type", async () => {
      render(<Modal isOpen={true} onClose={jest.fn()} type="rdv" />);
    ...
    test("should render with different titles based on mode and data", () => {
      const { rerender } = render(
        <Modal isOpen={true} onClose={jest.fn()} type="student" />
      );
    ...
    test("should close modal on close button click", async () => {
      ...
      render(
        <Modal isOpen={true} onClose={mockOnClose} type="student" mode="view" />
      );
