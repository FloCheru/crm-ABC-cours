...
export const PDFGenerator: React.FC<PDFGeneratorProps> = ({
  ...
  const loadPDFs = async () => {
    try {
      setIsLoading(true);
      const response = await ndrService.listPDFs(settlementNote._id);
  ...
  const handleGeneratePDF = async (type: "ndr" | "coupons" | "both") => {
    ...
    try {
      ...
      // Appel réel à l'API
      const result = await ndrService.generatePDF(
        settlementNote._id,
        type
      );
  ...
  const downloadPDF = async (pdfId: string) => {
    try {
      ...
      const blob = await ndrService.downloadPDF(
        settlementNote._id,
        pdfId
      );
  ...
  const previewPDF = async (pdfId: string) => {
    try {
      ...
      await ndrService.previewPDF(settlementNote._id, pdfId);
    } catch (error) {
  ...
  const deletePDF = async (pdfId: string) => {
    ...
    try {
      ...
      await ndrService.deletePDF(settlementNote._id, pdfId);
        ...
        <div className="flex gap-3 flex-wrap">
          <div>
            ...
            <div className="text-xs text-gray-500 mt-1">
              Document de facturation uniquement
            </div>
          </div>
        ...
        <div>
          ...
          {isLoading ? (
            ...
            <div className="text-gray-500 text-sm italic">
              Aucun PDF généré pour cette note
            </div>
          ) : (
                ...
                >
                  <div className="flex-1">
                    ...
                    <div className="text-xs text-gray-500 mt-1">
                      ...
                      {new Date(pdf.generatedAt).toLocaleDateString("fr-FR")}
                    </div>
                  </div>
