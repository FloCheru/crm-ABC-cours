 ...
 * Source: https://fr.wikipedia.org/wiki/Département_français
 */
...
/**
 * Liste complète des départements français triés par code numérique croissant
 * Ordre : 01-95 (métropole) -> 2A, 2B (Corse) -> 971-976 (DOM-TOM)
 */
...
export const getDepartmentByCode = (code: string): Department | undefined => {
  return FRENCH_DEPARTMENTS.find(dept => dept.code === code);
};

/**
 * Helper pour obtenir tous les départements d'une région
 */
 ...
 */
export const REGIONS = Array.from(new Set(FRENCH_DEPARTMENTS.map(d => d.region))).sort();
