...
class RdvService {
  ...
  async deleteRdv(
    id: string
  ): Promise<{ message: string; action: "cancelled" | "deleted" }> {
    try {
  ...
  async checkAvailability(
    ...
    try {
      ...
      const data = (await apiClient.get(
        `${this.baseUrl}/availability/${adminId}?${params.toString()}`
      )) as any;
  ...
  async getStats(): Promise<RdvStats> {
    try {
      ...
      console.error(
        "Erreur lors de la récupération des statistiques RDV:",
        error
      );
  ...
  async getRdvsByFamily(familyId: string): Promise<RendezVous[]> {
    try {
      ...
      console.error(
        "Erreur lors de la récupération des RDV de la famille:",
        error
      );
      throw this.handleError(
        error,
        "Impossible de récupérer les rendez-vous de la famille"
      );
    }
  }
  ...
  async getRdvsByAdmin(
    ...
    try {
      ...
      console.error(
        "Erreur lors de la récupération des RDV de l'admin:",
        error
      );
      throw this.handleError(
        error,
  ...
  async getUpcomingRdvs(): Promise<RendezVous[]> {
    try {
      ...
      throw this.handleError(
        error,
        "Impossible de récupérer les prochains rendez-vous"
      );
    }
  }
  ...
  private validateRdvData(data: Partial<CreateRdvData>): void {
    ...
    if (data.time) {
      ...
      if (hours < 8 || hours > 21 || minutes % 30 !== 0) {
        throw new Error(
          "L'heure doit être entre 08:00 et 21:00, par créneaux de 30 minutes"
        );
      }
    }
  ...
  getTimeUntilRdv(date: Date, time: string): string {
    ...
    if (days > 0) {
      ...
      return `Dans ${remainingMinutes} minute${
