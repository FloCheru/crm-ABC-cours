...
export const SettlementDetails: React.FC = () => {
  ...
  return (
    <div>
      <Navbar activePath={location.pathname} />
      <PageHeader 
        title={`Note de règlement n°${settlementNote
          ? settlementNote._id.substring(settlementNote._id.length - 8).toUpperCase()
          : 'Chargement...'}`}
        breadcrumb={[
          { label: "Dashboard", href: "/admin/dashboard" },
          { label: `NDR n°${settlementNote ? settlementNote._id.substring(settlementNote._id.length - 8).toUpperCase() : 'Chargement...'}` }
        ]}
        description={settlementNote ? `Créé le ${new Date(settlementNote.createdAt).toLocaleDateString("fr-FR")} • Modifié le ${new Date(settlementNote.updatedAt || settlementNote.createdAt).toLocaleDateString("fr-FR")}` : undefined}
        backButton={{ label: "Retour au dashboard", href: "/admin/dashboard" }}
      />
      <Container layout="flex-col">
          ...
          <>
            <Container layout="flex-col">
              <Container>
                ...
                {/* Bouton de retour maintenant géré par PageHeader */}
                <ButtonGroup
                  variant="single"
                  buttons={[
                    {
